...
struct fz_font_context_s
{
	...
	/* Registered font */
	int font_inited;
	_cdecl const char *(*pfz_lookup_base14_font)(const char *name, int *size);
	_cdecl const char *(*pfz_lookup_builtin_font)(const char *name, int is_bold, int is_italic, int *size);
	_cdecl const char *(*pfz_lookup_cjk_font)(int registry, int serif, int wmode, int *size, int *index);
	_cdecl const char *(*pfz_lookup_noto_font)(int script, int serif, int *size);
	_cdecl const char *(*pfz_lookup_noto_symbol_font)(int *size);
	_cdecl const char *(*pfz_lookup_noto_emoji_font)(int *size);
};
...
int
fz_register_extern_font(fz_context *ctx, const char *fn) {
    int (*pfz_validate)(int, int);
#if defined(_WIN32) || defined(_WIN64)
...
#define CHECK_PF(pf) do { if (!pf) return 3; } while(0);

    /* validate the version */
    pfz_validate = (void*)GetProcAddress(hmod, "validate"); CHECK_PF(pfz_validate);
    if (pfz_validate(0/*major*/, 0/*min*/))
        return 2;

    /* fix up the interfaces */
    ctx->font->pfz_lookup_base14_font = (void*)GetProcAddress(hmod, "lookup_base14_font");
    ctx->font->pfz_lookup_builtin_font = (void*)GetProcAddress(hmod, "lookup_builtin_font");
    ctx->font->pfz_lookup_cjk_font = (void*)GetProcAddress(hmod, "lookup_cjk_font");
    ctx->font->pfz_lookup_noto_font = (void*)GetProcAddress(hmod, "lookup_noto_font");
    ctx->font->pfz_lookup_noto_symbol_font = (void*)GetProcAddress(hmod, "lookup_noto_symbol_font");
    ctx->font->pfz_lookup_noto_emoji_font = (void*)GetProcAddress(hmod, "lookup_noto_emoji_font");

    CHECK_PF(ctx->font->pfz_lookup_base14_font);
    CHECK_PF(ctx->font->pfz_lookup_builtin_font);
    CHECK_PF(ctx->font->pfz_lookup_cjk_font);
    CHECK_PF(ctx->font->pfz_lookup_noto_font);
    CHECK_PF(ctx->font->pfz_lookup_noto_symbol_font);
    CHECK_PF(ctx->font->pfz_lookup_noto_emoji_font);

    ctx->font->font_inited = 1;
    // succeeded
    return 0;
#else
    /* unimplemented */
...
{
    if (!ctx->font->font_inited)
        return *size = 0, NULL;
    return ctx->font->pfz_lookup_base14_font(name, size);
}
...
fz_lookup_builtin_font(fz_context *ctx, const char *name, int is_bold, int is_italic, int *size)
{
    if (!ctx->font->font_inited)
        return *size = 0, NULL;
    return ctx->font->pfz_lookup_builtin_font(name, is_bold, is_italic, size);
}
...
fz_lookup_cjk_font(fz_context *ctx, int registry, int serif, int wmode, int *size, int *index)
{
    ...
    return ctx->font->pfz_lookup_cjk_font(registry, serif, wmode, size, index);
}
...
fz_lookup_noto_font(fz_context *ctx, int script, int serif, int *size)
{
    if (!ctx->font->font_inited)
        return *size = 0, NULL;
    return ctx->font->pfz_lookup_noto_font(script, serif, size);
}
...
fz_lookup_noto_symbol_font(fz_context *ctx, int *size)
{
    if (!ctx->font->font_inited)
        return *size = 0, NULL;
    return ctx->font->pfz_lookup_noto_symbol_font(size);
}
...
fz_lookup_noto_emoji_font(fz_context *ctx, int *size)
{
    if (!ctx->font->font_inited)
        return *size = 0, NULL;
    return ctx->font->pfz_lookup_noto_emoji_font(size);
}
