...
util.printd = function(fmt, d)
{
	...
	for (i = 0; i < length; i++)
	{
		switch(tokens[i])
		{
			case 'mmmm': res += uView.monthName[d.getMonth()]; break;
			case 'mmm': res += uView.monthName[d.getMonth()].substring(0,3); break;
			case 'mm': res += uView.padZeros(d.getMonth()+1, 2); break;
			case 'm': res += d.getMonth()+1; break;
			case 'dddd': res += uView.dayName[d.getDay()]; break;
			case 'ddd': res += uView.dayName[d.getDay()].substring(0,3); break;
			case 'dd': res += uView.padZeros(d.getDate(), 2); break;
			case 'd': res += d.getDate(); break;
			case 'yyyy': res += d.getFullYear(); break;
			case 'yy': res += d.getFullYear()%100; break;
			case 'HH': res += uView.padZeros(d.getHours(), 2); break;
			case 'H': res += d.getHours(); break;
			case 'hh': res += uView.padZeros((d.getHours()+11)%12+1, 2); break;
			case 'h': res += (d.getHours()+11)%12+1; break;
			case 'MM': res += uView.padZeros(d.getMinutes(), 2); break;
			case 'M': res += d.getMinutes(); break;
			case 'ss': res += uView.padZeros(d.getSeconds(), 2); break;
...
util.printx = function(fmt, val)
{
	...
	while (i < length)
	{
		switch (fmt.charAt(i))
		{
			...
			case '?':
				if (!val) return res;
				res += uView.convertCase(val.charAt(0),cs);
				val = val.substring(1);
				break;
			...
			default:
				res += uView.convertCase(fmt.charAt(i),cs);
				break;
		}
...
{
	var m = d.match(uView.monthPattern);

	return m ? uView.shortMonthName.indexOf(m[0]) : null;
}
...
function AFSimple_Calculate(op, list)
{
	...
	{
		var field = uView_Doc.getField(list[i]);
