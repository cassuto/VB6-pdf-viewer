...
static void *alloc(void *actx, void *ptr, int n)
{
	...
	if (ptr)
		return fz_resize_array(ctx, ptr, (unsigned)n, 1);
	return fz_malloc_array(ctx, (unsigned)n, 1);
}
...
int murun_main(int argc, char **argv)
{
	...
	js_setglobal(J, "uview");

	/* re-implement matrix math in javascript */
	js_dostring(J, "uview.Identity = [1,0,0,1,0,0];");
	js_dostring(J, "uview.Scale = function(sx,sy) { return [sx,0,0,sy,0,0]; };");
	js_dostring(J, "uview.Translate = function(tx,ty) { return [1,0,0,1,tx,ty]; };");
	js_dostring(J, "uview.Concat = function(a,b) { return ["
			"a[0] * b[0] + a[1] * b[2],"
			"a[0] * b[1] + a[1] * b[3],"
			"a[2] * b[0] + a[3] * b[2],"
			"a[2] * b[1] + a[3] * b[3],"
