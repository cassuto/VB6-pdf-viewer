...
/*
 * Create registry keys to associate uView with PDF and XPS files.
 */
...
{
	char buf[512];
	HKEY software, classes, uview, dotpdf, dotxps, dotepub;
	...
	OPEN_KEY(classes, "uView", uview);
	OPEN_KEY(uview, "SupportedTypes", supported_types);
	OPEN_KEY(uview, "shell", shell);
	OPEN_KEY(shell, "open", open);
	OPEN_KEY(open, "command", command);

	sprintf(buf, "\"%s\" \"%%1\"", argv0);

	SET_KEY(open, "FriendlyAppName", "uView");
	SET_KEY(command, "", buf);
	SET_KEY(supported_types, ".pdf", "");
	SET_KEY(supported_types, ".xps", "");
	SET_KEY(supported_types, ".epub", "");
	SET_KEY(pdf_progids, "uView", "");
	SET_KEY(xps_progids, "uView", "");
	SET_KEY(epub_progids, "uView", "");
...
{
	MessageBoxA(hwndframe, msg, "uView: Warning", MB_ICONWARNING);
}

void winerror(pdfapp_t *app, char *msg)
{
	MessageBoxA(hwndframe, msg, "uView: Error", MB_ICONERROR);
	exit(1);
}
...
{
	MessageBoxA(hwndframe, "The uView library supports printing, but this application currently does not", "Print document", MB_ICONWARNING);
}

int winsavequery(pdfapp_t *app)
{
	switch(MessageBoxA(hwndframe, "File has unsaved changes. Do you want to save", "uView", MB_YESNOCANCEL))
	{
...
int winfilename(wchar_t *buf, int len)
{
	...
	ofn.lpstrTitle = L"uView: Open PDF file";
...
int wingetsavepath(pdfapp_t *app, char *buf, int len)
{
	...
	ofn.lpstrInitialDir = NULL;
	ofn.lpstrTitle = L"uView: Save PDF file";
...
void winopen()
{
	...
	SetWindowTextW(hwndframe, L"uView");

	menu = GetSystemMenu(hwndframe, 0);
	AppendMenuW(menu, MF_SEPARATOR, 0, NULL);
	AppendMenuW(menu, MF_STRING, ID_ABOUT, L"About uView...");
...
{
	fprintf(stderr, "usage: uview [options] file.pdf [page]\n");
...
WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nShowCmd)
{
	...
	rc = fz_register_extern_font(ctx, "libuvfonts-tiny.dll");
	sprintf(buff, "rc=%d", rc);
	if (rc) {
	    MessageBoxA(NULL, buff, "fz_register_extern_font()", MB_OK);
	}
